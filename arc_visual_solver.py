...
"""
        ...
        for i in range(2, len(task['train'])):
            ...
            prompt_1 += f"""Here's training example {i+1}:
...
"""
        ...
        prompt_1 += f"""Now, here's the test input. Apply the pattern you've learned to generate the output.
...
IMPORTANT: Provide your answer as a grid in the exact same format, with square brackets and comma-separated values. Make sure the dimensions are correct."""
        ...
        response_test = self.call_ai_with_image(prompt_1, [])
        num_phases += 1
